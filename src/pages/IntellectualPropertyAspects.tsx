...
import { useIntellectualPropertyAspects } from "@/hooks/useIntellectualPropertyAspects";
import { calculateOverallRating } from "@/utils/intellectualPropertyScoring";

export default function IntellectualPropertyAspects() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const projectName = searchParams.get("project");
  const assessmentDate = searchParams.get("date");
  
  const { aspects, handleAspectClick, handleFindingsChange, handleOwnersChange } = useIntellectualPropertyAspects(projectName, assessmentDate);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log("Successfully saved intellectual property overall rating:", overallRating);
      toast.success("Intellectual Property aspects saved successfully");
      navigate('/');
    } catch (error) {
